...
\caption{Function values of $f(\cdot)$ and $\tilde{f}(\cdot)$ in Eq.(\ref{eq:f}) and Eq.(\ref{eq:f2}).}\label{fig:f}
\end{figure}
...
\subsection{Adjusting for unequal $k$-mer frequencies}

We assume each $k$-mer $t$ occurs at frequency $4^{-k}$ when deriving Eq.~(\ref{eq:S}).
This assumption does not hold because $k$-mer frequencies are directly correlated with the genome-wide GC content
which often deviates from 50\%.
We will extend equations in Section~\ref{sec:model} to more general cases.
...
$$
\log\tilde{P}(\vec{c}_x)=4^k\lambda(\log\lambda-1)-\sum_{t\in\Sigma^k}\log c_x(t)!+\sum_{t\in\Sigma^k}c_x(t)\log r_t
$$
The revised $H(\lambda)$ is more involved:
\begin{eqnarray*}
\tilde{H}(\lambda)&=&\sum_t\sum_{n=0}^{\infty}p(n|\lambda r_t)\log p(n|\lambda r_t)\\
&=&4^k\lambda(\log\lambda-1)+\lambda\sum_t r_t\log r_t - \tilde{f}(\lambda)
\end{eqnarray*}
where
\begin{equation}\label{eq:f2}
\tilde{f}(\lambda)=\sum_t e^{-\lambda r_t}\sum_{n=0}^{\infty}\log n!\cdot\frac{(\lambda r_t)^n}{n!}
\end{equation}
It may be tempted to define the complexity as follows
\begin{eqnarray*}
&&\tilde{H}(\lambda)-\log\tilde{P}(\vec{c}_x)-T\cdot\ell(x)\\
&=&\sum_t\log c_x(t)!-T\cdot\ell(x)-\tilde{f}(4^{-k}\ell(x))\\
&&-\left(\sum_tc_x(t)\log r_t-\ell(x)\sum_t q_t\log r_t\right)
\end{eqnarray*}
However, such a function mixes $k$-mer counts $c_x(t)$ and background $k$-mer frequency $r_t$.
A rare $k$-mer may contribute more to the function than a repetitive $k$-mer.
We could not find a theoretically clean solution.
Nonetheless, we note that the contribution of $r_t$ mainly comes from the last term in the parenthesis
and it approaches 0 under large $\ell(x)$ because $c_x(t)\approx\ell(x)q_t$.
We decide to ignore the term and define the complexity function as
$$
\tilde{S}(\vec{c}_x)=\sum_t\log c_x(t)!-T\cdot\ell(x)-\tilde{f}\left(\frac{\ell(x)}{4^k}\right)
$$
The only difference from Eq.~(\ref{eq:S}) is that $\tilde{S}$ uses $\tilde{f}$ instead of $f$.
As is shown in Fig.~\ref{fig:f}, $\tilde{f}$ is larger than $f$ when the GC content deviates from 50\%.
This effectively raises the threshold to call LC intervals.
